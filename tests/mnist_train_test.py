...
def build_iree_module(artifacts_dir,
  ...
  with open(os.path.join(artifacts_dir, "mnist.mlir"), "wb") as f:
  ...
  iree_vmfb_path = os.path.join(artifacts_dir, "mnist.vmfb")
...
def train_mnist_test(backend: str, runtime: str):
  ...
  with TemporaryDirectory() as tmp_dir:
    ...
    # Check intialization.
    # TODO: investigate why this fails on CUDA with wrong result.
    # Maybe it is uint32 vs int32.
    # On llvm-cpu it crashesh.
    # See: https://github.com/iree-org/iree/issues/11636
...
@pytest.mark.xfail
def test_train_mnist_cuda():
  train_mnist_test(backend="cuda", runtime="cuda")


@pytest.mark.xfail
def test_train_mnist_llvm_cpu():
