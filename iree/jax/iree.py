...
class IREE:
  ...
  def compiled_artifact(self):
    if not self._compiled_artifact:
      ...
      # We are passing the program as string and not as bytecode,
      # because a discrepancy between the MLIR version in Jax and IREE
      # causes the MLIR bytecode generated by Jax to not be valid in IREE.
      # Maybe this is intentional, but we are unable to bump jaxlib up
      # from 0.3.25. With version 0.4.1 it causes another problem with
      # dynamic shapes where PyRankedTensorType complains that fp32 is not
      # a floating point type. Strange.
      self._compiled_artifact = iree.compiler.tools.compile_str(
          input_str=str(ir_module),
